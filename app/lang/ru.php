...
return [
    ...
    'ml_ecosystem.sample_transformers_title' => 'Пример на TransformersPHP',
    'ml_ecosystem.sample_llphant_title' => 'Пример на LLPhant',
    ...
    'ml_ecosystem.rubix_intro' => 'Rubix поддерживает классификацию, регрессию, кластеризацию и работу с датасетами как first-class объектами. Посмотрим, как это выглядит на практике. <br>Допустим, у нас есть данные для бинарной классификации. Код ниже также обучает классификатор k-ближайших соседей (k-NN), но уже на данных роста и веса с метками пола, а затем предсказывает метку $M$ для нового человека. Для параметров  модель возвращает $[172, 68]$, так как среди 3 ближайших соседей большинство с этой меткой.',
    ...
    'ml_ecosystem.transformers_intro' => 'TransformersPHP предлагает простой pipeline API для задач вроде анализа настроений, классификации текста, семантического сравнения и т.д. В примере ниже модель определяет тональность двух фраз и показывает метку и score.',
    'ml_ecosystem.llphant_intro' => 'LLPhant – лёгкий фреймворк для вызова LLM из PHP. В примере ниже отправляем один prompt в OpenAI через LLPhant и выводим ответ модели.',
    ...
    'errors_loss.case5.pure_php_intro2' => 'В этом кейсе мы буквально "увидим" обучение без градиентного спуска и без "магии" библиотек: возьмём простую зависимость $y = 2x$, зададим модель $ŷ = wx$ и перебором найдём такой $w$, при котором MSE минимальна.',
    ...
    // Линейная регрессия: Кейс 4 (оценка вероятного чека клиента)
    'linear_regression.case4.intro1' => 'В интернет-магазинах, маркетплейсах и сервисах подписок часто важно заранее оценить, какой чек вероятнее всего принесёт конкретный клиент. Это помогает выбирать персональные предложения, скидки и сегментацию, а также приоритизировать работу с пользователями.',
    'linear_regression.case4.intro2' => 'Ниже – минимальный пример регрессионной модели, которая по поведению пользователя на сайте оценивает вероятный чек. Чтобы не получать отрицательные значения и сделать распределение более "ровным", будем предсказывать не сам чек, а его логарифм, а затем возвращаться к деньгам через экспоненту.',
    'linear_regression.case4.feature_x1' => '$x_1$ – количество визитов за период;',
    'linear_regression.case4.feature_x2' => '$x_2$ – время на сайте (секунды);',
    'linear_regression.case4.feature_x3' => '$x_3$ – количество просмотров страниц;',
    'linear_regression.case4.feature_x4' => '$x_4$ – скидка (в процентах);',
    'linear_regression.case4.formula' => 'Целевая переменная – логарифм чека: $y = \log(\text{check})$. Модель предсказывает $\hat{y}$, а затем мы восстанавливаем чек как $\widehat{\text{check}} = \exp(\hat{y})$.',
    'linear_regression.case4.rubix_intro' => 'В этом примере мы используем Ridge-регрессию из RubixML. Модель обучается на небольшом наборе наблюдений, предсказывает логарифм чека, а затем мы переводим результат обратно в деньги через exp().',
    'linear_regression.case4.explain_intro' => 'Почему логарифм чека – полезный приём:',
    'linear_regression.case4.explain_item1' => 'чек обычно имеет "тяжёлый хвост": редкие большие покупки сильно выделяются; логарифм сглаживает такие выбросы;',
    'linear_regression.case4.explain_item2' => 'модель в лог-пространстве часто лучше приближает мультипликативные эффекты (например, "в 2 раза больше просмотров");',
    'linear_regression.case4.explain_item3' => 'восстановление через exp() гарантирует положительный прогноз чека;',
    'linear_regression.case4.explain_item4' => 'Ridge-регрессия добавляет L2-регуляризацию и помогает не переобучаться на малом датасете.',
    'linear_regression.case4.explain_outro' => 'На практике вы бы расширили датасет и добавили признаки (канал трафика, категории интереса, историю покупок), но даже такой baseline помогает быстро получить работающий ориентир.',

    // Линейная регрессия: Кейс 5 (прогноз зарплаты по рынку)
    'linear_regression.case5.intro1' => 'Оценка зарплаты на рынке – типичная задача регрессии: по набору признаков кандидата (опыт, технологии, тип компании, удалёнка) мы хотим получить число – ожидаемую зарплату.',
    'linear_regression.case5.intro2' => 'Ниже – минимальный baseline на RubixML: обучаем Ridge-регрессию на маленьком датасете и затем предсказываем зарплату для нового кандидата. Это демонстрация механики, а не "реальная" модель рынка.',
    'linear_regression.case5.feature_x1' => '$x_1$ – опыт (в годах);',
    'linear_regression.case5.feature_x2' => '$x_2$ – "скор" технологий (условная числовая оценка стека);',
    'linear_regression.case5.feature_x3' => '$x_3$ – размер компании (уровень: 1 = small, 2 = medium, 3 = enterprise);',
    'linear_regression.case5.feature_x4' => '$x_4$ – удалёнка (1 = да, 0 = нет);',
    'linear_regression.case5.formula' => 'Модель линейной регрессии: $ŷ = w_0 + w_1 x_1 + w_2 x_2 + w_3 x_3 + w_4 x_4$, где $ŷ$ – прогноз зарплаты.',
    'linear_regression.case5.rubix_intro' => 'Используем Ridge-регрессию (линейная регрессия с L2-регуляризацией): она устойчива на маленьких датасетах и помогает не переобучаться.',
    'linear_regression.case5.explain_intro' => 'Как читать результат (веса и bias):',
    'linear_regression.case5.explain_item1' => 'вес при опыте показывает, насколько в среднем растёт зарплата на каждый дополнительный год;',
    'linear_regression.case5.explain_item2' => 'вес при "скоре" технологий отражает премию за более востребованный стек;',
    'linear_regression.case5.explain_item3' => 'вес при размере компании часто коррелирует с вилками компенсаций;',
    'linear_regression.case5.explain_item4' => 'вес при удалёнке может отражать поправку рынка (но в реальности это сильно зависит от региона и политики компании).',
    'linear_regression.case5.explain_outro' => 'В реальной задаче вы бы добавили больше данных, нормализацию признаков, категориальные признаки (регион, грейд, индустрия) и оценку качества модели на тестовой выборке.',
    ...
    'gradient_descent.implementation_intro' => 'В этом разделе мы реализуем градиентный спуск на PHP: сначала минимальную версию "с нуля", затем – более удобную векторную форму. При просмотре анимации используйте кнопки запуска, чтобы выполнить код и поэкспериментировать с параметрами.',
    'gradient_descent.run_animation' => 'Запустить анимацию',
    'gradient_descent.animation_title' => 'Реализация градиентного спуска (анимация)',
    'gradient_descent.animation_gd_title' => 'Градиентный спуск (анимация)',
    'gradient_descent.animation_intro' => 'Ниже – небольшая интерактивная анимация градиентного спуска в 1D, 2D и 3D. Настройте скорость обучения и стартовую точку, нажмите "Старт" и посмотрите, как параметры движутся к минимуму, а ошибка уменьшается.',
    'gradient_descent.animation.back_to_toc' => '← Назад к оглавлению',
    'gradient_descent.animation.ui.parameters' => 'Параметры',
    'gradient_descent.animation.ui.objective_1d' => 'Минимизируем \(f(x) = (x - a)^2\)',
    'gradient_descent.animation.ui.objective_2d' => 'Минимизируем \(f(x,y) = (x - a)^2 + (y - b)^2\)',
    'gradient_descent.animation.ui.objective_3d' => 'Минимизируем \(f(x,y,z) = (x - a)^2 + (y - b)^2 + (z - c)^2\)',
    'gradient_descent.animation.ui.objective_3d_surface' => 'Визуализируем поверхность \(z = f(x,y)\), где \(f(x,y) = (x - a)^2 + (y - b)^2\)',
    'gradient_descent.animation.ui.mode' => 'Режим',
    'gradient_descent.animation.ui.learning_rate' => 'Скорость обучения (α)',
    'gradient_descent.animation.ui.start_x' => 'Начальный x',
    'gradient_descent.animation.ui.start_y' => 'Начальный y',
    'gradient_descent.animation.ui.start_z' => 'Начальный z',
    'gradient_descent.animation.ui.target_a' => 'Цель a (минимум по x)',
    'gradient_descent.animation.ui.target_b' => 'Цель b (минимум по y)',
    'gradient_descent.animation.ui.target_c' => 'Цель c (минимум по z)',
    'gradient_descent.animation.ui.speed' => 'Скорость (шагов/сек)',
    'gradient_descent.animation.ui.iteration' => 'Итерация',
    'gradient_descent.animation.ui.current_x' => 'Текущий x',
    'gradient_descent.animation.ui.current_y' => 'Текущий y',
    'gradient_descent.animation.ui.current_z' => 'Текущий z',
    'gradient_descent.animation.ui.fx' => '$f(x)$',
    'gradient_descent.animation.ui.grad_abs' => '$|∇f|$',
    'gradient_descent.animation.ui.update_rule' => 'Правило обновления:',
    'gradient_descent.animation.ui.grad_1d' => 'где $∇f(x) = 2(x - a)$',
    'gradient_descent.animation.ui.grad_2d' => 'где $∇f(x,y) = (2(x-a), 2(y-b))$',
    'gradient_descent.animation.ui.grad_3d' => 'где $∇f(x,y,z) = (2(x-a), 2(y-b), 2(z-c))$',
    'gradient_descent.animation.ui.trajectory' => 'Траектория',
    'gradient_descent.animation.ui.trajectory_hint_1d' => 'Точка движется к минимуму (вертикальная линия – $a$).',
    'gradient_descent.animation.ui.trajectory_hint_2d' => 'Точка движется к минимуму (цель – точка $(a,b)$).',
    'gradient_descent.animation.ui.trajectory_hint_3d' => '3D: точка движется к цели $(a,b,c)$ в пространстве $(x,y,z)$.',
    'gradient_descent.animation.ui.trajectory_hint_3d_surface' => '3D поверхность: путь показан на поверхности \(z=f(x,y)\), а \((x,y)\) движется к цели $(a,b)$.',
    'gradient_descent.animation.ui.loss_chart' => 'График ошибки',
    'gradient_descent.animation.ui.loss_chart_hint' => 'На графике: значение $f$ по итерациям. Если $α$ слишком большое – возможны колебания.',
    'gradient_descent.animation.ui.chart_iteration' => 'итерация',
    'gradient_descent.animation.ui.chart_fx' => 'f(x)',
    'gradient_descent.animation.ui.chart_series_fx' => 'f(x)',
    ...
    'gradient_descent.sample1.goal' => 'Цель этого примера – увидеть, как параметр $w$ меняется от эпохи к эпохе во время обучения.',
    'gradient_descent.sample1.data_label' => 'Данные:',
    'gradient_descent.sample1.model_label' => 'Модель:',
    'gradient_descent.sample1.update_label' => 'Обновление параметра:',
    'gradient_descent.sample1.run_intro' => 'Таблица ниже – это траектория параметра $w$ при batch gradient descent на данных $y = 2x$.',
    'gradient_descent.sample1.ui.epoch' => 'Эпоха',
    'gradient_descent.sample1.ui.speed' => 'Скорость',
    'gradient_descent.sample1.ui.loss' => 'loss',
    'gradient_descent.sample1.ui.current' => 'текущая точка',
    'gradient_descent.sample1.ui.current-loss' => 'текущая точка (loss)',
    'gradient_descent.sample2.intro' => 'В этом примере мы запускаем один и тот же градиентный спуск с разными значениями learning rate и сравниваем, как меняется параметр $w$.',
    'gradient_descent.sample2.goal' => 'Цель – увидеть разницу между слишком маленьким шагом, нормальным шагом и слишком большим шагом, который приводит к расходимости.',
    'gradient_descent.sample2.compare_label' => 'Сравниваем learning rate:',
    'gradient_descent.sample2.run_intro' => 'Ниже показано, как ведёт себя параметр $w$ при трёх значениях learning rate на одном и том же наборе данных.',
    'gradient_descent.sample2.lr_slow' => 'слишком маленький шаг: обучение идёт медленно.',
    'gradient_descent.sample2.lr_good' => 'нормальный шаг: параметр быстро приближается к правильному значению.',
    'gradient_descent.sample2.lr_diverge' => 'слишком большой шаг: обновления становятся нестабильными и расходятся.',
    ...
    'probability_confidence.case2.scenario_title' => 'Сценарий',
    'probability_confidence.case2.scenario_intro' => 'Предположим, существует редкое заболевание. Известно следующее:',
    'probability_confidence.case2.scenario_item1' => 'Болезнь встречается у 1 человека из 1000.',
    'probability_confidence.case2.scenario_item2' => 'Чувствительность теста (вероятность положительного результата при наличии болезни) – 99%.',
    'probability_confidence.case2.scenario_item3' => 'Специфичность теста (вероятность отрицательного результата у здорового человека) – 95%.',
    'probability_confidence.case2.scenario_question' => 'Пациент сдает тест и получает положительный результат. Вопрос: какова вероятность того, что он действительно болен?',
    'probability_confidence.case2.scenario_note' => 'Интуитивный ответ часто звучит так: "примерно 99%". Но это неверно. Причина – в редкости события.',

    'probability_confidence.case3.scenario_title' => 'Сценарий',
    'probability_confidence.case3.scenario_intro' => 'У нас есть модель, которая должна различать три типа писем: обычные, промо и спам. Для каждого письма используются два простых признака: длина темы и количество ссылок.',
    'probability_confidence.case3.scenario_note' => 'В отличие от бинарной классификации, здесь модель должна распределить уверенность сразу между несколькими классами. Именно для этого удобно использовать SoftmaxClassifier.',
    ...
    'logistic_regression.intro' => 'До этого мы говорили о логистической регрессии как о модели: формула, сигмоида, вероятность, граница решения. Всё это важно, но само по себе остаётся теорией. Нужны кейсы, чтобы увидеть, как эта модель работает на реальных задачах. Здесь не будет "идеальных" примеров. Данные могут быть простыми или шумными, признаки очевидными или странными, решения не всегда однозначными. Это нормально. Именно так логистическая регрессия используется на практике.',
    ...
    'logistic_regression.case1.php_intro2' => 'Это как раз тот случай, где логистическая регрессия подходит особенно хорошо. Мы построим простую модель на чистом PHP, которая оценивает вероятность оттока по поведению пользователя, и посмотрим, как эта вероятность формируется из признаков.',
    ...
    'logistic_regression.case2.intro' => 'Здесь нет многомерного пространства, нет сложных признаков и нет пересекающихся факторов. Есть один признак и бинарное решение. И этого уже достаточно, чтобы увидеть всю логику логистической регрессии.<br><br><b>Цель кейса</b><br>Предсказать, подпишется ли пользователь на email-рассылку, исходя только из времени, проведенного на сайте.<br><br>Модель должна отвечать на два вопроса:<br>1) Какова вероятность подписки?<br>2) Где проходит граница между "скорее подпишется" и "скорее не подпишется"?',
    ...
    'why_naive_bayes_works.case1.rubix_intro' => 'Затем мы реализуем тот же пример с помощью RubixML и посмотрим, как та же логика выражается на уровне библиотеки.',
    'why_naive_bayes_works.case1.php_run_intro' => 'В этом примере мы реализуем наивный Байес вручную для пары простых категориальных признаков и посмотрим, какие log-probability score получаются для каждого класса.',
    'why_naive_bayes_works.case1.php_result_explanation' => 'Вывод показан как <strong>логарифмы</strong> вероятностных score для каждого класса. Чем больше значение (т.е. ближе к 0), тем вероятнее класс. Мы суммируем log-вероятности по признакам и применяем сглаживание Лапласа, поэтому после сортировки первым будет предсказанный класс.',
    ...
    'why_naive_bayes_works.case2.title' => 'Кейс 2. Спам-фильтр на словах (Bernoulli Naive Bayes)',
    'why_naive_bayes_works.case2.php_intro' => 'В этом кейсе мы рассмотрим <strong>Bernoulli Naive Bayes</strong> для текста: признак – это факт присутствия слова в письме (1/0), а не количество повторений. Важный момент – в формуле учитываются и отсутствующие слова, поэтому модель учитывает не только то, что слово встретилось, но и то, что другие слова не встретились.',
    'why_naive_bayes_works.case2.rubix_intro' => 'Затем реализуем тот же пример через RubixML: подготовим бинарные признаки (слово есть/нет), обучим Naive Bayes и получим предсказание для нового письма.',
    'why_naive_bayes_works.case2.php_run_intro' => 'Ниже код запуска: мы считаем априорные вероятности классов, строим словарь, применяем сглаживание Лапласа для Bernoulli и получаем log-score для классов spam/ham.',
    'why_naive_bayes_works.case2.php_result_explanation' => 'Результат – это log-score для каждого класса. Чем больше значение (ближе к 0), тем вероятнее класс. После сортировки первым будет предсказанный класс.<br><br><b>Как читать вывод</b><br>Сравниваем:<br><br>spam = {spam}<br>ham = {ham}<br><br>{spam} > {ham} → модель выбирает spam',
    'why_naive_bayes_works.case2.rubix_run_intro' => 'Здесь мы обучаем RubixML NaiveBayes на бинарных признаках (bag-of-words в формате 0/1) и предсказываем класс для нового набора слов.',
    'why_naive_bayes_works.case2.rubix_result_explanation' => 'RubixML возвращает предсказанную метку класса для входного вектора. По смыслу это та же схема: prior × likelihoods (сглаживание и работа с логарифмами делаются внутри библиотеки).',
    ...
    'knn_local_solutions.case2.intro' => 'Во втором кейсе мы рассмотрим использование алгоритма k-ближайших соседей не для классификации, а для регрессии. Если в предыдущем примере результатом был класс пользователя, то теперь результатом будет числовое значение – оценка стоимости квартиры.',
